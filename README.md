...
4. `supabase/migrations/0004_ai_conversations.sql`(メンター会話の永続化)
...
### Phase 4: AI中心の対話型プランニング

「ボタンを押すとAIが一方通行で生成する」のではなく、**メンターとの会話がアプリの中心**になりました。

- **チャットからの実行**: `/mentor` のメンターはツールを持つエージェントです。会話の中で
  - `create_tasks` — 「◯◯を金曜までにやりたい」→ 内容を確認してタスク登録
  - `update_task` — 「ES終わった」→ タスクを完了に
  - `generate_daily_plan` — 「今日の計画を立てて。午前は病院」→ 会話で聞いた事情を指示にまとめて計画を生成・再生成
  - `save_memory` — 「夜は集中できないタイプ」→ 長期記憶として保存
  を実行し、実行内容はチャット内にチップ(✅/📅/🧠)で表示されます
- **AIの関数呼び出し引数もZodで検証**してから実行します(AI出力を無検証でDBに書かない方針は維持)
- **会話の永続化**: 全メッセージが `mentor_messages` に保存され、リロード・端末をまたいでも続きから話せます
- **会話が次の計画に生きる**: 計画生成時に直近3日分の会話がプロンプトに入るため、チャットで伝えた事情・希望(「朝型に戻したい」「今週はES優先」)がその後の計画にも反映されます
- 今日画面の主導線は「💬 メンターと計画を立てる」。従来のワンタップ生成(⚡)も残っています
...
| `/checkin` | 朝チェックイン |
| `/review` | 夜レビュー |
| `/mentor` | AIメンターチャット(タスク登録・計画生成・記憶保存を会話で実行) |
